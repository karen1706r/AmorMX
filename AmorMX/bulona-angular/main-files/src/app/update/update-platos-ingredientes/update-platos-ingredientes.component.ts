    ...
    if (this.platoFormGroup.valid) {
        ...
        ingredientesArray.controls.forEach((control) => {
            ...
            if (ingrediente.isSelected) {
                // Solo añadimos los ingredientes seleccionados
        ...
        // Verifica que hay algo para actualizar o eliminar
        if (ingredientesAActualizar.length === 0 && ingredientesAEliminar.length === 0) {
            Swal.fire('Advertencia', 'No hay ingredientes para actualizar o eliminar.', 'warning');
            return;
        }
        ...
        // Asegúrate que la función editplatosingredientes espera los parámetros en el orden correcto
        this.ps.editplatosingredientes(payload.ingredientes, selectedPlato, payload.ingredientes_a_eliminar).subscribe({
            next: () => {
